...
module.exports = function(proto) {
  ...
  proto.ffprobe = function() {
    ...
    var source = ( input.isStream ) ? "pipe:0" : input.source;
    ...
    this._getFfprobePath(function(err, path) {
      ...
      // Spawn ffprobe
      var ffprobe = spawn(path, ['-show_streams', '-show_format'].concat(options, source));
      ...
      // Finally, pipe the input if it was a stream
      if (input.isStream) {
        input.source.resume();
        input.source.pipe(ffprobe.stdin);

        // Set stdin error handler on ffprobe (prevents nodejs catching the error, but
        // ffprobe will fail anyway, so no need to actually handle anything)
